...
decision and recovery paths, and one real-operator smoke scenario for controller adoption.
The fixture suite is the required CI gate; the real-operator suite is an opt-in/nightly
gate until the upstream OLM installation versions are pinned and supported together.

## Test environments

| Suite | Cluster | Catalog content | Purpose |
|---|---|---|---|
| `fixture` | kind + OLMv0 + OLMv1 | Local FBC fixture image built in CI | Deterministic coverage of V1, V2, V3, V4. |
| `real-operator` | the same kind cluster | OLMv0's installed OperatorHub catalog | Proves V5.2–V5.8 with a real deployed operator. |
...
catalog image/package/channel/CSV version. Record these in the workflow job summary.
...
4. Build and load the fixture catalog image into kind. Apply its OLMv0 `CatalogSource`,
   then migrate it with `migrate-catalogs-v0-to-v1` and wait for its `ClusterCatalog` to
   become `Serving=True`.
...
`make migration/test-e2e-fixture` runs only deterministic fixture scenarios. `make
migration/test-e2e-real-operator` runs only the smoke scenarios. Both use the Kind-generated
...
1. The `unit-test` workflow runs `make migration/test-unit`. The `migration-e2e` workflow runs fixture
   and live-operator jobs independently, each with its own Kind cluster and kubeconfig.
2. Run all three jobs for pull requests, merge queues, and pushes to `main`. Preserve E2E
   diagnostics and CLI coverage artifacts for failed jobs.
...
Snapshots in `e2e/migration/fixtures/snapshots/` are committed test data. To deliberately update that
...
git add e2e/migration/fixtures/snapshots/
git diff --cached
